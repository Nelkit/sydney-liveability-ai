...
```
\section*{LLM-Assisted Progress Log}   ← unnumbered, tracked via \llmupdate rows

\section{Executive Summary}             (1)   ~0.5 page
...
After every content addition, append a row to the `\begin{longtable}` in the LLM-Assisted Progress Log section using:

```latex
\llmupdate{YYYY-MM-DD}{LLM}{Section Name}{Draft}{One-line summary of what was added.}{Next action or Pending.}
```

Use today's date. Status options: `Draft`, `In Review`, `Done`.
