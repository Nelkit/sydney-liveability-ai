...
GTFS transport frequency scoring for Sydney suburbs.
...
Methodology:
For each MVP suburb defined in data/processed/suburbs.geojson:
...
2. For each stop, count the number of departures in the weekday peak window
   ...
   if absent, the filter is skipped (peak-hour departures are dominated by
   weekday services in practice).
...
5. Normalise to a 0–1 score relative to the best-served MVP suburb, so that
   the top suburb scores 1.0. This is a within-MVP comparison, not a
   city-wide absolute index.

Output:
data/processed/transport_scores.json, structured as:

    {
      "Newtown": {"transport_score": 0.91, "avg_services_per_hour": 14.2, "stop_count": 8},
      "Glebe":   {"transport_score": 0.74, "avg_services_per_hour": 11.6, "stop_count": 5}
    }

Consumed downstream by `/api/civic` and by Sprint 3's `ingest_transport.py`
which writes these rows into the `transport_scores` PostgreSQL table.
"""
...
# Configuration block
REPO_ROOT = Path(__file__).resolve().parent.parent  # resolve file path relative to repo root, not CWD
# GTFS raw files live outside OneDrive to avoid sync interference on 500MB+ stop_times.txt
RAW_DIR = Path(r"C:\dev\gtfs_cache")
PROCESSED_DIR = REPO_ROOT / "data" / "processed"
SUBURBS_FILE = PROCESSED_DIR / "suburbs.geojson"
...
def load_suburbs() -> gpd.GeoDataFrame:
    # Load MVP suburb polygons, strip (NSW) suffix, project to WGS84.
...
def load_stops_in_scope(suburbs_gdf: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
    # Load stops.txt and spatially join to suburbs.
...
def load_peak_stop_events(scope_stop_ids: set[str]) -> pd.DataFrame:
    ...
    for chunk in chunk_iter:
        ...
        if chunk.empty:
            continue
        # GTFS times can exceed 24h (e.g. "25:03:00"); first two chars are hour.
...
def filter_to_weekday_trips(peak_df: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        ...
        # if calendar.txt is absent, keep all trips; peak-hour departures are
        # dominated by weekday services in practice.
        pass
...
def score_suburbs(
    ...
    # Ensure every MVP suburb appears, even if zero stops
    ...
    if max_freq == 0:
        raise RuntimeError(
            "No peak-hour services matched to any suburb. "
            "Check that GTFS files are extracted and suburbs.geojson polygons overlap stops."
        )
...
def write_output(agg: pd.DataFrame) -> dict:
    # Serialise to the exact schema Sprint 3 expects.
    output = {}
...
def run() -> dict:
    ...
    print(f"  {len(suburbs_gdf)} suburb polygons loaded")

    print("Spatial-joining GTFS stops to suburbs")
    stops_in_scope = load_stops_in_scope(suburbs_gdf)
    print(f"  {len(stops_in_scope)} stops within MVP suburbs")
